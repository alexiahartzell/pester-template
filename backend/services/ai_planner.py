...
def process_inbox(inbox_tasks: list[TaskResponse]) -> str:
    ...
    return _chat(
        system=(
            ...
            "If the category is 'my meetings' or 'group meetings', you MUST set suggested_start_time "
            "and suggested_end_time (HH:MM format, 24h). If the user included a time in the title, extract it "
            "into the time fields and remove it from the title. If no time was given, set needs_clarification "
            "to true and ask what time the meeting is and how long it lasts. "
            "Do NOT accept a meeting without start and end times.\n"
            ...
            "suggested_start_time (HH:MM or null), suggested_end_time (HH:MM or null), "
            "suggested_difficulty ('easy', 'medium', or 'hard'), "
            "suggested_recurrence ('weekly', 'biweekly', 'monthly', or null), "
